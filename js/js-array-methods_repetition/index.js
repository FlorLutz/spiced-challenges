...
const onlyTitles = recipes.map((element) => element.title);
// ['Crepes', ...]

const titlesWithDuration = recipes.map(
  (element) => `${element.title} (${element.duration}min)`
);
// ['Crepes (60min)', ...]
...
const allTitlesInOneString = recipes.map((element) => element.title).join(", ");
